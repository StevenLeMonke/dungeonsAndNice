...
public class Render extends ApplicationAdapter
{
    ...
    public void render()
    {
        ...
        if(gamestate == GameState.GAME)
        {
            ...
            else if(y <= 2 && x <= 2)
            {
                for(int r = 0 ; r < 7 ; r ++)
                {
                    for(int c = 0 ; c < 7 ; c ++)
                    {
                        ...
                        if(r == y && c ==x)
                            batch.draw(player.texture(), (float) c * Constants.TILE_WIDTH, (float) (r + 1) * Constants.TILE_HEIGHT, Constants.TILE_WIDTH, Constants.TILE_HEIGHT);
                    }
                }
            }
            else if(y <= 2 && x > map.length - 3)
            {
                for(int r = 0 ; r < 7 ; r ++)
                {
                    for(int c = x - 3 ; c < x + 4 ; c ++)
            ...
            else if(y >= map.length - 3 && x <= 2)
            {
                for(int r = map.length - 7 ; r < map.length  ; r ++)
                {
                    for(int c = 0 ; c < 7 ; c ++)
                    {
                        batch.draw(map[r][c].getTexture(), (float) c * Constants.TILE_WIDTH, (float) (r  - (map.length - 7)) * Constants.TILE_HEIGHT);
                        if(r == y && c ==x)
                            batch.draw(player.texture(), (float) c * Constants.TILE_WIDTH, (float) (r  - (map.length - 7)) * Constants.TILE_HEIGHT, Constants.TILE_WIDTH, Constants.TILE_HEIGHT);
                    }
                }
            }
            else if(y >= map.length - 3 && x >= map.length - 3)
            {
                for(int r = map.length - 7 ; r < map.length  ; r ++)
                {
                    for(int c = map.length - 7 ; c < map.length; c ++)
                    {
                        batch.draw(map[r][c].getTexture(), (float) (c - (map.length - 7)) * Constants.TILE_WIDTH, (float) (r  - (map.length - 7)) * Constants.TILE_HEIGHT);
                        if(r == y && c ==x)
                            batch.draw(player.texture(), (float) (c - (map.length - 7)) * Constants.TILE_WIDTH, (float) (r  - (map.length - 7)) * Constants.TILE_HEIGHT, Constants.TILE_WIDTH, Constants.TILE_HEIGHT);
                    }
                }
            }
            else if(y <= 2)
            ...
            else if(x >= map.length - 3)
            {
                for(int r = y - 3 ; r < y + 4 ; r ++)
                {
                    for(int c = map.length - 7 ; c < map.length; c ++)
                    {
                        batch.draw(map[r][c].getTexture() ,(float) (c - (map.length - 7)) * Constants.TILE_WIDTH, (float) (r - (y - 3)) * Constants.TILE_HEIGHT);
                        System.out.println(" " + x + " , " + y);
                        if(r == y && c == x)
                            batch.draw(player.texture(),(float) (c - (map.length - 7)) * Constants.TILE_WIDTH, (float) (r - (y - 3)) * Constants.TILE_HEIGHT, Constants.TILE_WIDTH, Constants.TILE_HEIGHT); 
                    }
                }
            }
        }
